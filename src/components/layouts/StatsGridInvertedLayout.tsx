import type { StatsGridInvertedLayout as StatsGridInvertedLayoutType, CardContent } from "@/types"

const whiteCard = { background: "#ffffff", borderColor: "#ffffff" }
const purpleCard = { background: "#AA96F8", borderColor: "#AA96F8" }
const blueCard = { background: "#7774FF", borderColor: "#7774FF" }
...
type Props = {
  layout: StatsGridInvertedLayoutType
  accent: string
}

export default function StatsGridInvertedLayout({ layout }: Props) {
  const topRow = layout.cards.slice(0, 2)
  const bottomRow = layout.cards.slice(2, 4)

  return (
    <div className="relative mx-auto grid w-full items-stretch gap-5 px-4 py-8 sm:max-w-[90%] sm:px-6 md:grid-cols-[4fr_5fr] md:px-12 md:py-12 lg:px-20">
      {/* Grande carte gauche */}
      <div className="relative flex flex-col justify-center gap-8 overflow-hidden rounded-2xl p-6 md:gap-16 md:rounded-3xl md:p-10">
        <div className="pointer-events-none absolute -bottom-16 -right-16 h-80 w-80 rounded-full blur-[100px] bg-blue" />
        <div className="pointer-events-none absolute -top-40 -left-20 h-80 w-80 rounded-full blur-[100px] bg-blue" />
        <h2 className="big-text relative z-10 text-2xl leading-tight text-white sm:text-3xl md:text-4xl lg:text-5xl">
          {layout.leftCard.title}
        </h2>
        <div className="relative z-10 flex flex-col gap-4">
          {layout.leftCard.paragraphs.map((p, i) => (
          ...
          <div key={rowIdx} className="grid grid-cols-2 gap-4 md:gap-6">
            {row.map((card, i) => (
              <div
                ...
                style={card.color === "white" ? whiteCard : (card.color === "blue" ? blueCard : purpleCard)}
              >
