...
export default function StatsGridInvertedLayout({ layout }: Props) {
  ...
  return (
    <div className="relative mx-auto grid min-h-screen max-w-[90%] items-stretch gap-5 px-6 py-12 md:grid-cols-[4fr_5fr] md:px-12 lg:px-20">
      {/* Grande carte gauche */}
      <div className="relative flex flex-col justify-center gap-16 overflow-hidden rounded-3xl p-10">
        <div
          className="pointer-events-none absolute -bottom-16 -right-16 h-80 w-80 rounded-full blur-[100px] bg-blue"
        />
        <div
          className="pointer-events-none absolute -top-40 -left-20 h-80 w-80 rounded-full blur-[100px] bg-blue"
        />
      ...
      {/* Colonne droite : 2×2 cartes carrées */}
          ...
          <div key={rowIdx} className="grid grid-cols-2 gap-6">
            {row.map((card, i) => (
              <div
                ...
                style={card.color === "white" ? whiteCard : (
                  card.color === "blue" ? blueCard : purpleCard
                )}
              >
