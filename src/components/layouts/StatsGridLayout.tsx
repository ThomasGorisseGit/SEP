...
function CardContentView({ content }: { content: CardContent }) {
  if (content.type === "stat") {
    return (
      <div className="flex flex-col items-center text-center">
        <span className="big-text text-4xl leading-none p-2 md:text-5xl xl:text-5xl 2xl:text-7xl md:p-4" style={{ color: darkTextStrong }}>
          {content.value}
        </span>
        <p className="mt-2 text-sm leading-5 md:text-sm xl:text-xs 2xl:text-base md:leading-6" style={{ color: darkText }}>
          {content.label}
        </p>
        {content.source && (
  ...
  return (
    <blockquote className="text-sm leading-6 italic font-bold text-center md:text-base xl:text-sm 2xl:text-xl md:leading-8" style={{ color: darkTextStrong }}>
      {content.text}
      ...
      <div className="relative mx-auto grid w-full items-stretch gap-5 px-4 py-8 sm:max-w-[90%] sm:px-6 md:grid-cols-[5fr_4fr] md:px-12 md:py-12 lg:px-20">
        ...
        <div className="relative flex flex-col justify-center gap-8 overflow-hidden rounded-2xl p-6 md:gap-16 md:rounded-3xl md:p-10">
          ...
          <div
            ...
            style={{ backgroundColor: "#AA96F8" }}
          />
          <h2 className="big-text relative z-10 text-2xl leading-tight text-white sm:text-3xl md:text-3xl lg:text-3xl xl:text-3xl 2xl:text-5xl">
            {layout.rightCard.title}
          </h2>
          <div className="relative z-10 flex flex-col gap-4">
            {layout.rightCard.paragraphs.map((p, i) => (
              <p key={i} className="text-base leading-7 text-white md:text-base xl:text-base 2xl:text-xl md:leading-9">
                {p}
              </p>
            ))}
          </div>
